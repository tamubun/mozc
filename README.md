カナ入力のキー配置を、ローマ字テーブルを使って自由に設定出来るようにするパッチです。今のところ、本来のローマ字入力の方の動作を変えてしまっているので、ローマ字入力と併用することは出来ません。

使い方は、Mozc設定パネルでは、ローマ字入力にし、ユーザー定義のローマ字テーブルとして、カナ入力用のテーブルを与えてやるだけです。

本修正を使わずにオリジナルのMozcで同じ事をしようとすると、例えば、「か」と「が」にそれぞれ、"t", "t[" を割り当てている場合、preedit状態で「か」を入力しようとして"t"を打鍵したときに、まだこの後に"["が続く可能性があるので、「か」と表示されず、「ｔ」が表示されてしまうという不都合があります。
